...
| `before_install`                                        | After operator install policy runs, on staged skill/plugin install material. Explicitly trusted plugins declaring `activation.onCapabilities: ["hook"]` are loaded before dispatch.                                                                                                         |
...
- `before_install`: same terminal/no-op semantics as above. Multiple handlers merge findings in priority order and stop at the first block. Use `security.installPolicy` for operator-owned decisions because a plugin cannot inspect its own first installation and disabling it disables its hook.
