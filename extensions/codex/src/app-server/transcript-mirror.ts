...
async function mirror(params: {
  ...
  const mirrorBatch = await withSessionTranscriptMirrorWriteLock(
    ...
    async (transcript) => {
      ...
      for (const { dedupeIdentity, idempotencyKey, message, sourceFingerprint } of candidates) {
        ...
        const { messageCount, result: appended } = await transcript.appendMessageWithMessageCount({
          message: messageToAppend,
